...
bool CBoss::Init()	//Used when all the values are initialized by default
...
void CBoss::SetQTRoot(QuadtreeRoot *QTR)
{
	QTRoot = QTR;
	for (int l = 0; l<CPL_MAX_LASERS; l++)
...
void CBoss::ResizeDirectory(unsigned int S)
{
	unsigned int	i;
...
void CBoss::SetGameRef(CCharacter *Game)
{
	unsigned int i;
...
void CBoss::AssignTextMngr(CCharacter * TM)
{
	unsigned int	i;
...
*/
void CBoss::SetEndingCriteria(bool ByTime, bool ByFrame)
{
	unsigned int i;
...
//AI
void CBoss::AI_Init(void)	///Default Artificial Intelligence character initialization
{
	State = CPL_UNBORN;
}

void CBoss::AI_Explode(void)
{
...
void CBoss::AI_Die(void)	///Default Artificial Intelligence character initialization
{
	State = CPL_DYING;
...
void CBoss::ActivateLaser(CPL_BOSS_LASER LaserActivated)
{
...
//Physics
void CBoss::Collided(CCharacter *CollidedChar)
{
	unsigned int i;
...
void CBoss::ChangeRenderMode(CHAR_RENDER_MODE Mode)
{
	int i, l;
...
void CBoss::MoveTo(float x, float y, float z)	///Moves to the absolute coordinate x,y,z
{
	Vector P;
...
void CBoss::MoveTo(Vector &Move)				///Moving to an absolute position. It does not matter where the character is
...
void CBoss::MoveRelTo(float x, float y, float z)	///Moves x,y,z units from the to the current position
{
	Vector P;
...
void CBoss::MoveRelTo(Vector &Move)	///Moving relative to the character current Position
{
...
void CBoss::ManageEvent(unsigned int Event, bool On)
{
	switch (Event)
	{
...
void CBoss::DiscreteUpdate(void)
{
	double		msUpdPlayer;
...
void CBoss::ReceiveMessage(RTDESK_CMsg *pMsg)
{
	DiscreteUpdate();
}

void CBoss::UserShoot()
{
...
void CBoss::SetRenderPeriod(HRT_Timems RP)
{
	unsigned int i;
...
void CBoss::SetUpdatePeriod(HRT_Timems UP)
{
	unsigned int i;
