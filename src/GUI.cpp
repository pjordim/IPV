...
*/
void CGUI::Transform(void)
{
	glTranslatef(Position.v[XDIM], Position.v[YDIM], Position.v[ZDIM]);
	glRotatef(Rotation.v[XDIM], 1.0, 0.0, 0.0);
	glRotatef(Rotation.v[YDIM], 0.0, 1.0, 0.0);
	glRotatef(Rotation.v[ZDIM], 0.0, 0.0, 1.0);
...
{
	Position = { 0.0f, 0.0f, -SIGLBD_DEFAULT_Z_POSITION };// -.05 in order to avoid z-fighting
...
{
	glColor4f(0.0f, 0.0f, 0.0f, GUI.Fading);

	Position.v[XDIM] = 0.1f*LoadingBMPPosition;
	Position.v[YDIM] = 1.1f*LoadingBMPPosition;
	Position.v[ZDIM] = SIGLBD_DEFAULT_Z_POSITION;

	RenderQuad(CTM_LOADING, 2.56*LoadingBMPPosition, 1.44f*LoadingBMPPosition);
}
