...
bool COverboss::Init()	//Used when all the values are initialized by default
...
void COverboss::SetQTRoot(QuadtreeRoot *QTR)
{
	QTRoot = QTR;
	for (int l = 0; l<CPL_MAX_LASERS; l++)
...
void COverboss::ResizeDirectory(unsigned int S)
{
	unsigned int	i;
...
void COverboss::SetGameRef(CCharacter *Game)
{
	unsigned int i;
...
void COverboss::AssignTextMngr(CCharacter * TM)
{
	unsigned int	i;
...
*/
void COverboss::SetEndingCriteria(bool ByTime, bool ByFrame)
{
	unsigned int i;
...
//AI
void COverboss::AI_Init(void)	///Default Artificial Intelligence character initialization
{
	State = CPL_UNBORN;
}

void COverboss::AI_Explode(void)
{
...
void COverboss::AI_Die(void)	///Default Artificial Intelligence character initialization
{
	State = CPL_DYING;
...
void COverboss::ActivateReactor()
{
	Vector ReactorShift;
...
void COverboss::ActivateLaser(CPL_OVERBOSS_LASER LaserActivated)
{
...
//Physics
void COverboss::Collided(CCharacter *CollidedChar)
{
	unsigned int i;
...
void COverboss::ChangeRenderMode(CHAR_RENDER_MODE Mode)
{
	int i, l;
	...
	case CHAR_NO_RENDER:			//No render for this character: camera, collision objects,...
		return;
	case CHAR_2D:
		Position.v[YDIM] = CPL_OVERBOSS_Y_2D;
		ShootType = CSH_PLAYER2D;
	...
	case CHAR_LINES3D:
	case CHAR_3D:
		Position.v[YDIM] = CPL_OVERBOSS_Y_3D;
...
void COverboss::MoveTo(float x, float y, float z)	///Moves to the absolute coordinate x,y,z
{
	Vector P;
...
void COverboss::MoveTo(Vector &Move)				///Moving to an absolute position. It does not matter where the character is
...
void COverboss::MoveRelTo(float x, float y, float z)	///Moves x,y,z units from the to the current position
{
	Vector P;
...
void COverboss::MoveRelTo(Vector &Move)	///Moving relative to the character current Position
{
...
void COverboss::ManageEvent(unsigned int Event, bool On)
{
	switch (Event)
	{
...
void COverboss::UserRight(bool On)
{
...
void COverboss::UserLeft(bool On)
{
	PositionPrev = Position;
	if (Alive())
		switch (RenderMode)
...
void COverboss::DiscreteUpdate(void)
{
	double		msUpdPlayer;
...
void COverboss::ReceiveMessage(RTDESK_CMsg *pMsg)
{
	DiscreteUpdate();
}

void COverboss::UserShoot()
{
...
void COverboss::SetRenderPeriod(HRT_Timems RP)
{
	unsigned int i;
...
void COverboss::SetUpdatePeriod(HRT_Timems UP)
{
	unsigned int i;
